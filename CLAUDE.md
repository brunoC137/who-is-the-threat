...
GET    /api/decks                            supports query filters; hides archived unless
                                             ...
                                             `includeArchived=true` (both); default limit
                                             100, newest first; `withUsage=true` adds
                                             per-deck `gamesPlayed` / `lastPlayedAt`,
                                             derived from games
...
- **Deck selection** — every place a deck is chosen uses `components/DeckPicker.tsx` (search, recently played, own vs. borrowed tabs, archived handling), fed by `decksAPI.getAll({ limit: 500, withUsage: true })`. Do not reintroduce a native `<select>` of decks.
